...
# DEMARCATIONS
data <-data %>% 
  mutate(position = if_else(player %in% c('1'), "GK", 
                            if_else( (team %in% c('team A') & player %in% c('2','4','5','11'))|
                                       (team %in% c('team B') & player %in% c('3','5','6','11')),"DF",
                                     if_else( (team %in% c('team A') & player %in% c('3','8','10','9'))|
                                                (team %in% c('team B') & player %in% c('2','4','8','9')),"MD",
                                              if_else( (team %in% c('team A') & player %in% c('6','7'))|
                                                         (team %in% c('team B') & player %in% c('7','10')),"FW",
                                                       "BA"))))
  )
...
data_hull_position <- data %>%
  filter(player != 1) %>% 
  group_by(team,position, time) %>%
  nest() %>%
  mutate(
    hull = map(data, ~ with(.x, chull(x, y))),
    out = map2(data, hull, ~ .x[.y,,drop=FALSE])
...
## good test 1 ##
  ...
  pitch_plot(68, 105) +
  geom_point(data = filter(data, str_detect(team, 'team B')), aes(x, y, group = player, fill = position), shape = 21, size = 6, stroke = 2) +
  geom_point(data = filter(data, team == 'ball'), aes(x, y), shape = 21, fill = 'dark orange', size = 4) +
  geom_line(data = filter(data, str_detect(team, 'team B') & str_detect(position, 'DF')  ),aes(x=min(x),y),colour='blue')+
  geom_line(data = filter(data, str_detect(team, 'team B') & str_detect(position, 'DF')  ),aes(x=max(x),y),colour='blue')+
  geom_line(data = filter(data, str_detect(team, 'team B') & str_detect(position, 'DF')  ),aes(x=x,y=min(y)),colour='blue')+
  geom_line(data = filter(data, str_detect(team, 'team B') & str_detect(position, 'DF')  ),aes(x=x,y=max(y)),colour='blue')+
   # transition_time(time) +
  
  scale_fill_manual(values = c('GK'='red','DF'='blue','MD'='green','FW'='black'  )) +
  geom_text(data = filter(data, str_detect(team, 'team B')), aes(x, y, label = player), color = 'white') +
  guides(fill = FALSE)
...
pitch_plot(68, 105) +
  geom_point(data = filter(data, str_detect(team, 'team A')), aes(x, y, group = player, fill = position), shape = 21, size = 6, stroke = 2) +
  geom_point(data = filter(data, team == 'ball'), aes(x, y), shape = 21, fill = 'dark orange', size = 4) +
  #geom_polygon(data = filter(data, str_detect(team, 'team B') & str_detect(position, 'MD')  ),aes(x,y),fill='blue')+
  geom_polygon(data = filter(data_hull_position, team == 'team A'), aes(x, y,fill = position), alpha = 0.4) +
  # transition_time(time) +
  
  # scale_fill_manual(values = c('GK'='red','DF'='blue','MD'='green','FW'='black'  )) +
  geom_text(data = filter(data, str_detect(team, 'team A')), aes(x, y, label = player), color = 'white') +
  guides(fill = FALSE)
